...
export default function LocationCoverage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [currentResponse, availableResponse] = await Promise.all([
        API.guideDashboard.getGuideLocations(),
        API.guideDashboard.getAvailableLocations(),
      ]);
  ...
  const handleAddLocations = async () => {
    ...
    try {
      ...
      const response = await API.guideDashboard.updateGuideLocations(
        updateData
      );
  ...
  const handleRemoveLocation = async (location, type) => {
    ...
    try {
      const response = await API.guideDashboard.checkLocationRemoval(
        location.name
      );
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        subtitle="Manage the locations where you provide your guide services"
        actions={
          <button
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                className="bg-cyan-600 text-white px-4 py-2 rounded-lg hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                {submitting && (

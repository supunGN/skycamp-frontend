...
export default function TripBookings() {
  ...
  const fetchBookings = async () => {
    try {
      setLoading(true);
      const response = await API.guideDashboard.getGuideBookings();
  ...
  const handleMarkAsFinished = (booking) => {
    showConfirmationModal(
      "Mark as Finished",
      `Are you sure you want to mark booking #${booking.booking_id} as finished? This will complete the trip and move it to past bookings.`,
      async () => {
        try {
          setMarkingFinished(true);
          const response = await API.guideDashboard.markBookingAsFinished(
            booking.booking_id
          );

          if (response.success) {
            showSuccess("Trip marked as finished successfully!");
            fetchBookings(); // Refresh the bookings list
          } else {
            showError(response.message || "Failed to mark trip as finished");
          }
        } catch (error) {
          console.error("Error marking trip as finished:", error);
          showError("Failed to mark trip as finished");
        } finally {
    ...
    >
      ...
      <div className="space-y-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-gray-600">
            <strong>Trip Date:</strong> {formatDate(booking.start_date)}
          </span>
        </div>
      ...
      {booking.status === "Confirmed" && (
        <div className="pt-4 border-t border-gray-200">
          <button
            onClick={() => handleMarkAsFinished(booking)}
            disabled={markingFinished}
            className="w-full bg-cyan-600 text-white px-4 py-2 rounded-lg hover:bg-cyan-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
          >
            {markingFinished ? (
              <>
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        title="Trip Bookings"
        subtitle="Manage your guide service bookings"
      />
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          ...
          >
            <div className="flex items-center gap-2">
              <CheckCircleIcon className="h-4 w-4" />
              Past Trips ({bookings.past_bookings.length})
            </div>
          </button>
        </nav>
      </div>

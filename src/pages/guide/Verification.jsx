...
const Verification = () => {
  ...
  useEffect(() => {
    const loadVerificationData = async () => {
      try {
        setLoading(true);
        const response = await API.guideDashboard.getVerificationDocs();
  ...
  const handleSubmitVerification = async () => {
    ...
    try {
      ...
      const response = await API.guideDashboard.submitVerification(formData);
  ...
  const handleSubmitExistingVerification = async () => {
    ...
    try {
      ...
      const response = await API.guideDashboard.submitVerification(
        new FormData()
      );
  ...
  const getVerificationStatusInfo = () => {
    ...
    if (status === "Yes") {
      ...
      return {
        icon: InformationCircleIcon,
        text: "Upload both sides of your NIC to verify your identity and unlock premium features",
        color: "cyan",
        bgColor: "cyan-50",
        borderColor: "cyan-200",
      };
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <CheckCircleIcon className="w-4 h-4 text-green-600" />
              <span className="text-sm text-gray-700">
                Access to verified guide features
              </span>
            </div>
          </div>
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <CheckCircleIcon className="w-4 h-4 text-green-600" />
              <span className="text-sm text-gray-700">
                Higher priority in guide listings
              </span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircleIcon className="w-4 h-4 text-green-600" />
              <span className="text-sm text-gray-700">
                Enable premium guide features
              </span>
            </div>
          </div>
        </div>
